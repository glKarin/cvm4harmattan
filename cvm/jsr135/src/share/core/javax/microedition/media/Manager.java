...
public final class Manager {
    ...
    public static String[] getSupportedProtocols(String content_type) {
        // TODO
        debug("getSupportedProtocols(" + content_type + ")");
    ...
    public static Player createPlayer(String locator) throws IOException, MediaException {
        ...
        if (locStr.startsWith("capture://radio")) {
            throw new MediaException("Radio Capture is not supported");
        } else if (locStr.startsWith("capture://audio")) {
            throw new MediaException("Audio Capture is not supported");
        } else if (locStr.startsWith("capture://video")) {
            throw new MediaException("Video Capture is not supported");
        } else if (locStr.startsWith("device://tone")) {
            return new TonePlayer();
        } else if (locStr.startsWith("device://midi")) {
        ...
        String type = null;
        String filename = locator.toLowerCase();
        ...
        if (type != null) {
            MaemoMediaPlayer player = new MaemoMediaPlayer(locator, type);
            return player;
        }

        if (type != null) {
            try {
                StreamConnection connection = (StreamConnection)Connector.open(locator);
    ...
    public static Player createPlayer(InputStream stream, String type) throws IOException, MediaException {
        // TODO
        debug("createPlayer(" + stream + ", " + type + ")");
        ...
        if (!found) {
            throw new MediaException("Player cannot be created for type " + type);
        }

        if (type.equals("audio/x-wav")) {
            MaemoMediaPlayer player = new MaemoMediaPlayer(stream, type);
            return player;
        }
        ...
        if (type.equals("audio/x-tone-seq")) {
            TonePlayer player = new TonePlayer();
            player.open(stream, type);
            return player;
        }
    ...
    public static void playTone(int note, int duration, int volume) throws MediaException {
        if (note < 0 || note > 127 || duration <= 0) {
            throw new IllegalArgumentException("Invalid note (" + note + ") or duration (" + duration + ")");
        }
        ...
        MaemoMediaPlayer.playTone(note, duration, volume);
    }
    ...
    public static Player createPlayer(DataSource source) throws IOException, MediaException {
        ...
        throw new MediaException("Player cannot be created datasource " + source);
    }
    ...
    private static DataSource createDataSource(String locator) throws IOException, MediaException {
        // TODO
        debug("createDataSource(" + locator + ")");
