...
(JNIEnv* aEnv, jclass _unused, jlong aHandle, jint aFirst, jint aCount, jshortArray aSrcArray)
{
    int srcLength = 0;
    unsigned short *srcData = NULL;
    if (aSrcArray)
    {
        srcData = (unsigned short *)(*aEnv)->GetShortArrayElements(aEnv, aSrcArray, NULL);
        if (srcData == NULL)
        {
    ...
    srcLength = aSrcArray ? (*aEnv)->GetArrayLength(aEnv, aSrcArray) : 0;
    ...
    if (aSrcArray)
    {
        (*aEnv)->ReleaseShortArrayElements(aEnv, aSrcArray, (jshort*)srcData, JNI_ABORT);
    }
}

JNIEXPORT jlong JNICALL Java_javax_microedition_m3g_VertexArray__1ctor
(JNIEnv* aEnv, jclass _unused, jlong aM3g, jint aNumVertices, jint aNumComponents, jint aComponentSize)
{
    M3G_DO_LOCK
    jlong va = (jlong)m3gCreateVertexArray((M3GInterface)aM3g,
                        ...
                        M3G_INT);
		printf("=========contruct VertexArray: %d, %d, %d -> %ld\n", aNumVertices, aNumComponents, aComponentSize , va);
    M3G_DO_UNLOCK(aEnv)
    return (jlong) va;
}

JNIEXPORT void JNICALL Java_javax_microedition_m3g_VertexArray__1setByte
(JNIEnv* aEnv, jclass _unused, jlong aHandle, jint aFirst, jint aCount, jbyteArray aSrcArray)
{
    int srcLength = 0;
    unsigned char *srcData = NULL;
    if (aSrcArray)
    {
        srcData = (unsigned char *)(*aEnv)->GetByteArrayElements(aEnv, aSrcArray, NULL);
        if (srcData == NULL)
        {
            M3G_RAISE_EXCEPTION(aEnv, "java/lang/OutOfMemoryError");
            return;
    ...
    if (aSrcArray)
    {
        (*aEnv)->ReleaseByteArrayElements(aEnv, aSrcArray, (jbyte*)srcData, JNI_ABORT);
    }
}
...
(JNIEnv* aEnv, jclass _unused, jlong aHandle, jint aFirstVertex, jint aNumVertices, jbyteArray aSrcArray)
{
    int dstLength = 0;
    unsigned char *dstData = NULL;
    if (aSrcArray)
    {
        dstData = (unsigned char *)(*aEnv)->GetByteArrayElements(aEnv, aSrcArray, NULL);
        if (dstData == NULL)
        {
            M3G_RAISE_EXCEPTION(aEnv, "java/lang/OutOfMemoryError");
            return;
    ...
    dstLength = aSrcArray ? (*aEnv)->GetArrayLength(aEnv, aSrcArray) : 0;
    ...
    */
    ...
    if (aSrcArray)
    {
        ...
        (*aEnv)->ReleaseByteArrayElements(aEnv, aSrcArray, (jbyte*)dstData, 0);
    }
}

JNIEXPORT void JNICALL Java_javax_microedition_m3g_VertexArray__1getShort
(JNIEnv* aEnv, jclass _unused, jlong aHandle, jint aFirstVertex, jint aNumVertices, jshortArray aSrcArray)
{
    int dstLength = 0;
    unsigned short *dstData = NULL;
    if (aSrcArray)
    {
        dstData = (unsigned short *)(*aEnv)->GetShortArrayElements(aEnv, aSrcArray, NULL);
        if (dstData == NULL)
        {
            M3G_RAISE_EXCEPTION(aEnv, "java/lang/OutOfMemoryError");
            return;
    ...
    */
    ...
    {
        /* copy dstData array to java side and release both arrays */
        (*aEnv)->ReleaseShortArrayElements(aEnv, aSrcArray, (jshort*)dstData, 0);
    }
}
