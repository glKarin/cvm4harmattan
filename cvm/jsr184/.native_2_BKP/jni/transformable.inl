...
(JNIEnv* aEnv, jclass _unused, jlong aHandle, jfloat aAngle, jfloat aAx, jfloat aAy, jfloat aAz)
{
    M3G_DO_LOCK
...
(JNIEnv* aEnv, jclass _unused, jlong aHandle, jbyteArray aDstArray)
{
    if (validateArray(aEnv, aDstArray, sizeof(M3GMatrix)))
    {
        jbyte* dstArray = (*aEnv)->GetByteArrayElements(aEnv, aDstArray, NULL);
        if (dstArray == NULL)
        {
        ...
        M3G_DO_UNLOCK(aEnv)
        (*aEnv)->ReleaseByteArrayElements(aEnv, aDstArray, dstArray, 0);
    }
}

JNIEXPORT void JNICALL Java_javax_microedition_m3g_Transformable__1setTransform
(JNIEnv* aEnv, jclass _unused, jlong aHandle, jbyteArray aSrcArray)
{
    if (aSrcArray != NULL)
    {
        jbyte* srcArray = (*aEnv)->GetByteArrayElements(aEnv, aSrcArray, NULL);
        if (srcArray == NULL)
        {
            M3G_RAISE_EXCEPTION(aEnv, "java/lang/OutOfMemoryError");
            return;
        ...
        M3G_DO_UNLOCK(aEnv)
        (*aEnv)->ReleaseByteArrayElements(aEnv, aSrcArray, srcArray, JNI_ABORT);
    }
    else
    {
        M3G_DO_LOCK
...
(JNIEnv* aEnv, jclass _unused, jlong aHandle, jfloat aTx, jfloat aTy, jfloat aTz, jboolean aAbsolute)
{
...
(JNIEnv* aEnv, jclass _unused, jlong aHandle, jfloat aAngle, jfloat aAx, jfloat aAy, jfloat aAz, jboolean aAbsolute)
{
...
(JNIEnv* aEnv, jclass _unused, jlong aHandle, jfloat aSx, jfloat aSy, jfloat aSz, jboolean aAbsolute)
{
...
(JNIEnv* aEnv, jclass _unused, jlong aHandle, jfloatArray aDstArray)
{
    if (validateArray(aEnv, (jbyteArray)aDstArray, 3))
    {
        jfloat* dstArray = (*aEnv)->GetFloatArrayElements(aEnv, aDstArray, NULL);
        if (dstArray == NULL)
        {
            M3G_RAISE_EXCEPTION(aEnv, "java/lang/OutOfMemoryError");
            return;
