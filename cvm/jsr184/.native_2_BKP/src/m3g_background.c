...
static void m3gApplyBackground(RenderContext *ctx, Background *background)
{
    ...
    if (background->colorClearEnable) {
            ...
            background->modeY == M3G_BORDER)
        {
            ...
            qglClearColorx(temp[0], temp[1], temp[2], temp[3]);
        }
    }
    ...
    {
        {
            ...
            qglTexCoordPointer(2, GL_SHORT, 0, texvert);
            qglEnable(GL_TEXTURE_2D);
            qglTexEnvx(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, (GLfixed) GL_REPLACE);
            ...
            if (background->modeX == M3G_REPEAT) {
                qglTexParameterx(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
            }
            else {
                qglTexParameterx(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
            }
    
            if (background->modeY == M3G_REPEAT) {
                qglTexParameterx(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
            }
            else {
                qglTexParameterx(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
            }

            /* Texture matrix scale */
            qglMatrixMode(GL_TEXTURE);
            qglLoadIdentity();
            qglScalef(   m3gRcp((M3Gfloat)m3gGetWidth(background->image)),
            ...
            qglVertexPointer(3, GL_FIXED, 0, vert);
