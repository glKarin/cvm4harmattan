                                   ...
                                   M3GNativeBitmap bitmapHandle)
{
	...
	printf("+==================ggggggggg %x      %d %d|%s %s %s %s\n", error, config, numConfigs, qglGetString(GL_VERSION), qglGetString(GL_VENDOR), qglGetString(GL_EXTENSIONS), qglGetString(GL_RENDERER));
                return config;
            }
...
#define READPIXELS_BUFFER_SIZE      16384
        ...
        if (width > 0 && height > 0) {
            ...
            M3Gint bufSize = (width * 2 > READPIXELS_BUFFER_SIZE ? width * 2 : READPIXELS_BUFFER_SIZE);
            M3Gint numLinesInBuffer = bufSize/(width * 2);
            ...
            for (row = 0; row < height; row += numLinesRead) {
                ...
                while (line-- > 0) {
                    //m3gConvertPixels(M3G_RGB565, &temp[2*line*width], format, dst, width);
					//k
					memcpy(dst, temp + (2 * line * width), 2 * width);
                    dst += stride;
                }
            }
            m3gFreeTemp(M3G_INTERFACE(ctx));
                                     ...
                                     EGLSurface surface)
{
	...
	printf("=====eeeeeeeee %x |%s %s %s %s\n", eglGetError(), qglGetString(GL_VERSION), qglGetString(GL_VENDOR), qglGetString(GL_EXTENSIONS), qglGetString(GL_RENDERER));
                return rc->handle;
            }
            else {
