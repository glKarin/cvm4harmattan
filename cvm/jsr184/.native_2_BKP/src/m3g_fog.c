...
static void m3gApplyFog(const Fog *self)
{
	if (self != NULL) {
		...
		case M3G_LINEAR_FOG:
			...
			qglFogf(GL_FOG_START, self->start);
			qglFogf(GL_FOG_END, self->end);
			qglFogxv(GL_FOG_COLOR, temp);
			break;
		case M3G_EXPONENTIAL_FOG:
			qglEnable(GL_FOG);
			qglFogf(GL_FOG_MODE, GL_EXP);
			qglFogf(GL_FOG_DENSITY, self->density);
			qglFogxv(GL_FOG_COLOR, temp);
			break;
		}
	}
...
static void m3gApplySpriteFog(const Fog *self, M3Gfloat eyeZ, M3Gfloat finalZ)
{
    if(self != NULL) {
		...
		qglFogf(GL_FOG_START, -m3gDiv(finalZ, fogValue));
		qglFogf(GL_FOG_END, 0.f);
		qglFogxv(GL_FOG_COLOR, temp);
    }
    else {
		qglDisable(GL_FOG);
