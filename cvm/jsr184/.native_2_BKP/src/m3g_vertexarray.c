...
static void m3gLockColorArray(const VertexArray *array, M3Gint alphaFactor)
{
    ...
    if (alphaFactor >= 0x10000) {
        ...
        qglColorPointer(type == GL_UNSIGNED_BYTE ? 4 : array->elementSize,
                       type,
                       array->stride,
        ...
        qglColorPointer(4, GL_UNSIGNED_BYTE, 0, cache);
    }
    M3G_ASSERT_GL;
...
static void m3gLockNormalArray(const VertexArray *array)
{
    ...
    qglNormalPointer(array->elementType, array->stride,
...
static void m3gLockTexCoordArray(const VertexArray *array)
{
    ...
    qglTexCoordPointer(array->elementSize,
                      array->elementType,
                      array->stride,
                      m3gMapObject(M3G_INTERFACE(array), array->data));
    M3G_ASSERT_GL;
...
static void m3gLockVertexArray(const VertexArray *array)
{
    ...
    qglVertexPointer(array->elementSize,
                    array->elementType,
                    array->stride,
                    m3gMapObject(M3G_INTERFACE(array), array->data));
    M3G_ASSERT_GL;
                                            ...
                                            M3Gdatatype type)
{
...

    {
        ...
        if (!array->data) {
			...
			printf("@zzzzzzzz %p - %p\n", ptr, (char *)ptr + count * array->stride);
