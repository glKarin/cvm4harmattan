...
public class Camera extends Node {
	...
	public Camera() {
		...
		Object3D.KarinDebugCall(this, "public Camera() {");
	}
	...
	Camera(long handle) {
		super(handle);
		Object3D.KarinDebugCall(this, "Camera(long handle) {");
	}
	...
	public void setParallel(float height, float aspectRatio, float near, float far) {
		Object3D.KarinDebugCall(this, "public void setParallel(float height, float aspectRatio, float near, float far) {");
		_setParallel(handle, height, aspectRatio, near, far);
		System.out.println("setParallel " + handle + " " + height + " " + aspectRatio + " " + near + " " + far);
	}

	public void setPerspective(float fovy, float aspectRatio, float near, float far) {
		Object3D.KarinDebugCall(this, "public void setPerspective(float fovy, float aspectRatio, float near, float far) {");
		_setPerspective(handle, fovy, aspectRatio, near, far);
	}

	public void setGeneric(Transform transform) {
		Object3D.KarinDebugCall(this, "public void setGeneric(Transform transform) {");
		System.out.println("java setGeneric -> " + handle + " " + transform.matrix);
		_setGeneric(handle, transform.matrix);
	}

	public int getProjection(Transform transform) {
		Object3D.KarinDebugCall(this, "public int getProjection(Transform transform) {");
		return _getProjectionAsTransform(handle, transform != null ? transform.matrix : null);
	}

	public int getProjection(float[] params) {
		Object3D.KarinDebugCall(this, "public int getProjection(float[] params) {");
