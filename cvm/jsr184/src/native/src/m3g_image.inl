...
static GLenum m3gGetGLFormat(M3GPixelFormat format)
{
	...
	case M3G_RGB565: //k Nokia N9/50 GPU support R5G6B5
    case M3G_RGB8:
    case M3G_RGB8_32:
    case M3G_BGR8_32:
        return GL_RGB;
...
static void m3gBindTextureObject(Image *img, M3Gbool mipmap)
{
     ...
     * doesn't exist yet. */
    {
        if (!img->texObject) {
            GLint err;
            qglGenTextures(1, &img->texObject);
            err = qglGetError();
        ...
        qglBindTexture(GL_TEXTURE_2D, img->texObject);
    }
    ...
    if (img->dirty || (mipmap && img->mipmapsDirty)) {
        ...
        if (img->dirty) {
            ...
            if (img->paletteBytes > 0) {
                ...
                qglCompressedTexImage2D(GL_TEXTURE_2D,
                                       0,
                                       img->glFormat,
    ...
    
    {
        ...
        if (img->internalFormat == M3G_RGBA8) {
            ...
            for (row = 0; row < img->height; ++row) {
                qglReadPixels(0, img->height - row - 1,
                             img->width, 1,
