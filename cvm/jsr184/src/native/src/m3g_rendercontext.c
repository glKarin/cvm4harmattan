...
static void m3gSetGLDefaults(void)
{
	...
	qglLightModelfv(GL_LIGHT_MODEL_AMBIENT, black);
...
//EGL_ALPHA_SIZE, 0,	
		...
		display = qeglGetDisplay(EGL_DEFAULT_DISPLAY);	
		m3gInitializeGL(M3G_INTERFACE(ctx));	
		//eglInitialize(display, NULL, NULL);	
		//eglBindAPI(EGL_OPENGL_ES_API);	
		qeglChooseConfig(display, config_attribs, &config, 1, &num_configs);	
		context = qeglCreateContext(display, config, EGL_NO_CONTEXT, NULL);	
		surface = qeglCreatePbufferSurface(display, config, pbuffer_attribs);	
		qeglMakeCurrent(display, surface, surface, context);	
...
#   endif
    ctx->target.buffered = M3G_TRUE; //k TODO for test
...
static M3G_INLINE void m3gApplyLights(RenderContext *ctx, M3Gint scope)
{
    if (ctx->lastScope != scope) {
        ...
        if (ctx->renderMode == RENDER_NODES) {
            qglPushMatrix();
            qglLoadMatrixf(ctx->viewTransform);
        }
...
static void m3gInitRender(M3GRenderContext context, M3Genum renderMode)
{
    ...
    qglViewport(ctx->viewport.x, ctx->viewport.y,
               ctx->viewport.width, ctx->viewport.height);
	qglDepthRangef(ctx->depthNear, ctx->depthFar);
    qglScissor(ctx->scissor.x, ctx->scissor.y,
                                     ...
                                     M3Gbool newAlphaWrite)
{
    ...
    if (ctx->currentColorWrite != ctx->currentAlphaWrite) {
        ...
        qglScissor(ctx->clip.x0, ctx->clip.y0,
        ...
        /* Blend the texture with the frame buffer */
        {
            ...
            qglTexEnvx(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
    ...
    if (newColorWrite != newAlphaWrite) {
        ...
        qglCopyTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA,
        ...
        if (err == GL_INVALID_OPERATION) {
            ...
            qglCopyTexImage2D(GL_TEXTURE_2D, 0, GL_RGB,
                             ctx->clip.x0, ctx->clip.y0,
                             pow2Width, pow2Height,
                             0);
            err = qglGetError();
...
static void m3gPushScreenSpace(RenderContext *ctx, M3Gbool realPixels)
{
    ...
    if (realPixels) {
        ...
        qglOrthox(0, w << 16, 0, h << 16, -1 << 16, 1 << 16);
    }
    qglMatrixMode(GL_MODELVIEW);
    qglPushMatrix();
    qglLoadIdentity();
...
static void m3gClearInternal(RenderContext *ctx, Background *bg)
{
    ...
    qglColorMask(GL_TRUE, GL_TRUE, GL_TRUE, m3gGetAlphaWrite(ctx));
    qglDepthRangef(ctx->depthNear, ctx->depthFar);
    qglViewport(ctx->viewport.x, ctx->viewport.y,
               ctx->viewport.width, ctx->viewport.height);
    qglScissor(ctx->scissor.x, ctx->scissor.y,
