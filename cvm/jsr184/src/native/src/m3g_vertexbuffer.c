...
static void m3gApplyScaleAndBias(const VertexBuffer *buffer)
{
    ...
    qglMatrixMode(GL_TEXTURE);
    {
        ...
        for (i = 0; i < M3G_NUM_TEXTURE_UNITS; ++i) {
            if (buffer->texCoords[i] != NULL) {
                qglActiveTexture((GLenum)(GL_TEXTURE0 + i));
                qglTranslatef(buffer->texCoordBias[i][0],
                             buffer->texCoordBias[i][1],
                             buffer->texCoordBias[i][2]);
                qglScalef(buffer->texCoordScale[i],
    ...
    if (buffer->vertices != NULL) {
        qglTranslatef(buffer->vertexBias[0],
                     buffer->vertexBias[1],
                     buffer->vertexBias[2]);
        qglScalef(buffer->vertexScale,
                 buffer->vertexScale,
                 buffer->vertexScale);
    }
}
...
static void m3gGetBoundingBox(VertexBuffer *vb, AABB *boundingBox)
{
    ...
    *boundingBox = vb->bbox;
	printf("AABB bounding box: %f %f %f|%f %f %f\n", boundingBox->min[0], boundingBox->min[1], boundingBox->min[2], boundingBox->max[0], boundingBox->max[1], boundingBox->max[3]);
}
