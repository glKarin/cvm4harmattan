...
(JNIEnv* aEnv, jclass _unused, jlong aHandle, jint aIndex, jint aTime, jfloatArray aValue)
{
    jfloat* elems = NULL;
    if (aValue)
    {
        elems = (*aEnv)->GetFloatArrayElements(aEnv, aValue, NULL);
        if (elems == NULL)
        {
    ...
    jsize length = aValue ? (*aEnv)->GetArrayLength(aEnv, aValue) : 0;
    ...
    if (elems)
        (*aEnv)->ReleaseFloatArrayElements(aEnv, aValue, elems, 0);
}

JNIEXPORT void JNICALL Java_javax_microedition_m3g_KeyframeSequence__1setRepeatMode
(JNIEnv* aEnv, jclass _unused, jlong aHandle, jint aMode)
{
    M3G_DO_LOCK
...
(JNIEnv* aEnv, jclass _unused, jlong aHandle, jint aDuration)
{
    M3G_DO_LOCK
...
(JNIEnv* aEnv, jclass _unused, jlong aM3g, jint aNumKeyframes, jint aNumComponents, jint aInterpolation)
{
    M3G_DO_LOCK
...
(JNIEnv* aEnv, jclass _unused, jlong aHandle, jint aFirst, jint aLast)
{
    M3G_DO_LOCK
...
(JNIEnv* aEnv, jclass _unused, jlong aHandle, jint aIndex, jfloatArray aValue)
{
    jfloat* elems = NULL;
    if (aValue)
    {
            ...
            (aValue != NULL))
    {
        if (elems)
        {
            (*aEnv)->ReleaseFloatArrayElements(aEnv, aValue, elems, JNI_ABORT);
        }
