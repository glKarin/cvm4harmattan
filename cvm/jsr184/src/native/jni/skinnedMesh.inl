...
(JNIEnv* aEnv, jclass _unused, jlong aHandle, jlong aHBone, jint aWeight, jint aFirstVertex, jint aNumVertices)
{
    M3G_DO_LOCK
...
(JNIEnv* aEnv, jclass _unused, jlong aM3g, jlong aHVertices, jlongArray aHTriangles, jlongArray aHAppearances, jlong aHSkeleton)
{
    ...
    if (aHTriangles)
    {
        triangles = (*aEnv)->GetLongArrayElements(aEnv, aHTriangles, NULL);
        if (triangles == NULL)
        {
            if (appearances)
            {
    ...
    jint numTriangles = (*aEnv)->GetArrayLength(aEnv, aHTriangles);
...
(JNIEnv* aEnv, jclass _unused, jlong aHandle, jlong aBone, jbyteArray aTransform)
{
    jbyte *transform = NULL;
    if (aTransform)
    {
        transform = (*aEnv)->GetByteArrayElements(aEnv, aTransform, NULL);
        if (transform == NULL)
        {
            M3G_RAISE_EXCEPTION(aEnv, "java/lang/OutOfMemoryError");
            return;
    ...
    if (transform)
    {
        ...
        (*aEnv)->ReleaseByteArrayElements(aEnv, aTransform, transform, 0);
    }
}

JNIEXPORT jint JNICALL Java_javax_microedition_m3g_SkinnedMesh__1getBoneVertices
(JNIEnv* aEnv, jclass _unused, jlong aHandle, jlong aBone, jintArray aIndices, jfloatArray aWeights)
{
    int *indices = NULL;
    float *weights = NULL;
    jint vertices = 0;
    ...
    if (aIndices != NULL)
    {
        indices = (*aEnv)->GetIntArrayElements(aEnv, aIndices, NULL);
        if (indices == NULL)
        {
            M3G_RAISE_EXCEPTION(aEnv, "java/lang/OutOfMemoryError");
            return 0;
    ...
    if (aWeights != NULL)
    {
        weights = (*aEnv)->GetFloatArrayElements(aEnv, aWeights, NULL);
        if (weights == NULL)
        {
            if (indices)
            {
                /* Release indices int array*/
                (*aEnv)->ReleaseIntArrayElements(aEnv, aIndices, indices, JNI_ABORT);
            }
            M3G_RAISE_EXCEPTION(aEnv, "java/lang/OutOfMemoryError");
            return 0;
        }
    ...
    if (requiredLength == 0)
    {
        ...
        {
            /* Release indices int array */
            (*aEnv)->ReleaseFloatArrayElements(aEnv, aWeights, weights, JNI_ABORT);
        }
        return 0;
    }
    /*
    ...
    */
    ...
    {
        /* Update array to java side and release arrays */
        (*aEnv)->ReleaseIntArrayElements(aEnv, aIndices, indices, 0);
    }
    if (weights)
    {
        /* Update array to java side and release arrays */
        (*aEnv)->ReleaseFloatArrayElements(aEnv, aWeights, weights, 0);
    }
    return vertices;
}
