...
(JNIEnv* aEnv, jclass _unused, jlong aM3g, jlong aHVertices, jlongArray aHTriangles, jlongArray aHAppearances)
{
    ...
    int appearancesLen = aHAppearances ? (*aEnv)->GetArrayLength(aEnv, aHAppearances) : 0;
    ...
    jlong* triangle = (*aEnv)->GetLongArrayElements(aEnv, aHTriangles, NULL);
    if (triangle == NULL)
    {
    ...
    if (aHAppearances)
    {
        appearance = (*aEnv)->GetLongArrayElements(aEnv, aHAppearances, NULL);
        if (appearance == NULL)
        {
            (*aEnv)->ReleaseLongArrayElements(aEnv, aHTriangles, triangle, JNI_ABORT);
    ...
    if (appearance)
    {
        (*aEnv)->ReleaseLongArrayElements(aEnv, aHAppearances, appearance, JNI_ABORT);
    }
...
(JNIEnv* aEnv, jclass _unused, jlong aHandle, jint aIndex, jlong aHAppearance)
{
    M3G_DO_LOCK
