...
(JNIEnv* aEnv, jclass _unused, jlong aHImage2D, jint aX, jint aY, jint aWidth, jint aHeight, jbyteArray aImageArray)
{
    jbyte* imageArray = NULL;
    if (aImageArray)
    {
        imageArray = (*aEnv)->GetByteArrayElements(aEnv, aImageArray, NULL);
        if (imageArray == NULL)
        {
    ...
    M3G_DO_LOCK
    m3gSetSubImage((M3GImage)aHImage2D, aX, aY, aWidth, aHeight, aImageArray ? (*aEnv)->GetArrayLength(aEnv, aImageArray) : 0, imageArray);
    M3G_DO_UNLOCK(aEnv)

    if (imageArray)
    {
        (*aEnv)->ReleaseByteArrayElements(aEnv, aImageArray, imageArray, JNI_ABORT);
    }
}
...
jlong
#endif
JNICALL Java_javax_microedition_m3g_Image2D__1ctorImage
(JNIEnv* aEnv, jclass _unused, jlong aHM3g, jint aFormat, 
#if 0
 jint 
#else
 jlong
...
(JNIEnv* aEnv, jclass _unused, jlong aHImage2D)
{
    M3G_DO_LOCK
...
(JNIEnv* aEnv, jclass _unused, jlong aM3g, jint aFormat, jint aWidth, jint aHeight, jbyteArray aImage, jbyteArray aPalette)
{
    ...
    if (validateArray(aEnv, aImage, aWidth * aHeight))
    {
        ...
        int paletteLen = (*aEnv)->GetArrayLength(aEnv, aPalette);
        ...
        else
        {
            ...
            if (hImg != NULL)
            {
                ...
                jbyte* image = (*aEnv)->GetByteArrayElements(aEnv, aImage, NULL);
                if (image == NULL)
                {
                    M3G_RAISE_EXCEPTION(aEnv, "java/lang/OutOfMemoryError");
                    M3G_DO_UNLOCK(aEnv)
                ...
                palette = (*aEnv)->GetByteArrayElements(aEnv, aPalette, NULL);
                if (palette == NULL)
                {
                    if (image)
                    {
                        (*aEnv)->ReleaseByteArrayElements(aEnv, aImage, image, JNI_ABORT);
                    }
                    M3G_RAISE_EXCEPTION(aEnv, "java/lang/OutOfMemoryError");
                    M3G_DO_UNLOCK(aEnv)
                    return 0;
                ...
                if (palette)
                {
                    (*aEnv)->ReleaseByteArrayElements(aEnv, aPalette, palette, JNI_ABORT);
                }
            }
            M3G_DO_UNLOCK(aEnv)
            return ((jlong) hImg);
...
(JNIEnv* aEnv, jclass _unused, jlong aM3g, jint aFormat, jint aWidth, jint aHeight)
{
    M3G_DO_LOCK
...
(JNIEnv* aEnv, jclass _unused, jlong aM3g, jint aFormat, jint aWidth, jint aHeight, jbyteArray aImage)
{
    M3GImageFormat format = (M3GImageFormat)aFormat;
...
#endif
            {
                (*aEnv)->GetByteArrayRegion(aEnv, aImage, aWidth * i * bpp, aWidth * bpp, imageScanline);
