...
//k #include <android/bitmap.h>
#include <gxj_putpixel.h> //k
extern gxj_screen_buffer gxj_free_screen_buffer; //qt4/fb_port.c
#define false 0
...
(JNIEnv* _unused /*aEnv*/, jclass _unused_2)
{
/*    EGLContext ctx;
    EGLConfig config;
    EGLSurface surf;
 ...
 * Must be executed in UI thread
 */
static char *gl_pixels = NULL;
void * gl_get_m3g_pixel_data(void)
{
	return gl_pixels;
}
JNIEXPORT jboolean JNICALL Java_javax_microedition_m3g_Graphics3D__1bindGraphics
(JNIEnv* aEnv, jclass _unused, jlong aCtx, jlong aSurfaceHandle, jint aWidth, jint aHeight,
 ...
 jboolean aDepth, jint aHintBits, jboolean aIsProperRenderer, jobject bitmap)
{
    ...
    */
                            ...
                            M3G_COLOR_BUFFER_BIT) && m3gSetRenderHints((M3GRenderContext)aCtx, aHintBits))
    {
		...
		if(!gl_pixels)
		{
			gl_pixels = malloc(gxj_system_screen_buffer.width * gxj_system_screen_buffer.height * 2);
			memset(gl_pixels, 0x99, gxj_system_screen_buffer.width * gxj_system_screen_buffer.height * 2);
		}
		aWidth = gxj_system_screen_buffer.width;
		aHeight = gxj_system_screen_buffer.height;
		//pixels_ptr = gl_pixels;
		pixels_ptr = gxj_system_screen_buffer.pixelData;
		const M3GPixelFormat Pixel_Format = M3G_RGB565;
		const int Pixel_Size = 2;
	    m3gBindMemoryTarget((M3GRenderContext)aCtx, pixels_ptr, aWidth, aHeight, Pixel_Format, (M3Guint)(aWidth * Pixel_Size), 0);
		printf("UNIMPL->m3gBindGraphics  %d %d %p|%d %d %d %d\n", aWidth, aHeight, pixels_ptr, aClipX, aClipY, aClipW, aClipH);
#endif
...
(JNIEnv* aEnv, jclass _unused, jlong aHContext, jint aX, jint aY,
...
(JNIEnv* aEnv, jclass _unused, jlong aHandle,
 jlong aSurfaceHandle, jboolean _unused_2 /*aIsImageTarget*/, jboolean _unused_3 /*aIsProperRenderer*/, jobject bitmap)
{
    M3G_DO_LOCK
    ...
    int ret = 0; //k AndroidBitmap_unlockPixels(aEnv, bitmap);
    pixels_ptr = NULL;
    if (ret < 0) {
...
(JNIEnv* aEnv, jclass _unused, jlong aHContext, jlong aHCamera, jbyteArray aTransform)
{
    M3GMatrix *transform = NULL;
    if (aTransform)
    {
        transform = (Matrix *)((*aEnv)->GetByteArrayElements(aEnv, aTransform, NULL));
        if (transform == NULL)
        {
    ...
    if (transform)
    {
        (*aEnv)->ReleaseByteArrayElements(aEnv, aTransform, (jbyte*)transform, JNI_ABORT);
    }
}
...
(JNIEnv* aEnv, jclass _unused, jlong aHContext, jlong aHWorld)
{
    M3G_DO_LOCK
...
(JNIEnv* aEnv, jclass _unused, jlong aHContext,
 jlong aHVertices, jlong aHIndices, jlong aHAppearance, jbyteArray aTransform, jint aScope)
{
    M3GMatrix *transform = NULL;
    if (aTransform)
    {
        transform = (M3GMatrix *)(*aEnv)->GetByteArrayElements(aEnv, aTransform, NULL);
        if (transform == NULL)
        {
            M3G_RAISE_EXCEPTION(aEnv, "java/lang/OutOfMemoryError");
            return;
...
(JNIEnv* aEnv, jclass _unused, jlong aCtx, jlong aBg)
{
    M3G_DO_LOCK
...
(JNIEnv* aEnv, jclass _unused, jlong aHCtx)
{
    M3G_DO_LOCK
...
(JNIEnv* aEnv, jclass _unused, jlong aObject)
{
    M3G_DO_LOCK
...
(JNIEnv* aEnv, jclass _unused, jlong aHContext, jlong aHLight, jbyteArray aTransform)
{
    M3GMatrix *transform = NULL;
    if (aTransform)
    {
...
(JNIEnv* aEnv, jclass _unused, jlong aHCtx, jlong aImageHandle, jboolean aDepth, jint aHintBits)
{
    M3G_DO_LOCK
...
(JNIEnv* aEnv, jclass _unused, jlong aHContext, jfloat aDepthNear, jfloat aDepthFar)
{
    M3G_DO_LOCK
...
(JNIEnv* aEnv, jclass _unused, jlong aHContext, jint aLightIndex, jlong aHLight, jbyteArray aTransform)
{
    M3GMatrix *transform = NULL;
    if (aTransform)
    {
...
(JNIEnv* aEnv, jclass _unused, jlong aHCtx, jlong aHNode, jbyteArray aTransform)
{
    M3GMatrix *transform = NULL;
    if (aTransform)
    {
...
(JNIEnv* aEnv, jclass _unused, jintArray aStatArray)
{
    const M3Gint *statArray = (M3Gint *)(aStatArray != NULL ? (*aEnv)->GetIntArrayElements(aEnv, aStatArray, NULL) : NULL);
    jint statArrayLength = aStatArray ? (*aEnv)->GetArrayLength(aEnv, aStatArray) : 0;
    ...
    if (statArray)
    {
        (*aEnv)->ReleaseIntArrayElements(aEnv, aStatArray, (jint*)statArray, 0);
    }
...
(JNIEnv* aEnv, jclass _unused, jlong aHCtx, jbyteArray aTransform)
{
    M3GMatrix *transform = NULL;
    if (aTransform)
    {
    ...
    if (transform)
    {
        ...
        (*aEnv)->ReleaseByteArrayElements(aEnv, aTransform, (jbyte*)transform, 0);
    }
}
...
(JNIEnv* aEnv, jclass _unused, jlong aHCtx, jint aLightIndex, jbyteArray aTransform)
{
    M3GMatrix *transform = NULL;
    if (aTransform)
    {
...
(JNIEnv* _unused /*aEnv*/, jclass _unused_2, jlong aM3g)
{
