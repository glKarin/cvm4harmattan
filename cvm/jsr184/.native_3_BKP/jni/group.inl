...
(JNIEnv* aEnv, jclass _unused, jlong aHandle, jlong aHNode)
{
    M3G_DO_LOCK
...
(JNIEnv* aEnv, jclass _unused, jlong aHandle, jint aMask, jfloat aX, jfloat aY, jlong aHCamera, jfloatArray aResult)
{
    jfloat* elems = NULL;
    if (aResult)
    {
        elems = (*aEnv)->GetFloatArrayElements(aEnv, aResult, NULL);
        if (elems == NULL)
        {
    ...
    if (aResult)
        (*aEnv)->ReleaseFloatArrayElements(aEnv, aResult, elems, 0);
    return ret;
}
...
(JNIEnv* aEnv, jclass _unused, jlong aHandle, jint aMask, jfloatArray aRay, jfloatArray aResult)
{
    jfloat* rayElems = NULL;
    if (aRay)
    {
        rayElems = (*aEnv)->GetFloatArrayElements(aEnv, aRay, NULL);
        if (rayElems == NULL)
        {
            M3G_RAISE_EXCEPTION(aEnv, "java/lang/OutOfMemoryError");
            return 0;
    ...
    if (aResult)
    {
        resultElems = (*aEnv)->GetFloatArrayElements(aEnv, aResult, NULL);
        if (resultElems == NULL)
        {
            if (rayElems)
                (*aEnv)->ReleaseFloatArrayElements(aEnv, aRay, rayElems, JNI_ABORT);
            M3G_RAISE_EXCEPTION(aEnv, "java/lang/OutOfMemoryError");
            return 0;
        }
    }
    ...
    if (resultElems)
        (*aEnv)->ReleaseFloatArrayElements(aEnv, aResult, resultElems, 0);
    if (rayElems)
        (*aEnv)->ReleaseFloatArrayElements(aEnv, aRay, rayElems, 0);
    return ret;
}
