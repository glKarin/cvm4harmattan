                                   ...
                                   M3GNativeBitmap bitmapHandle)
{
	...
	printf("++++++++(rbga %d)%d %d %d %d\n", format, attribs[0].value, attribs[1].value, attribs[2].value, attribs[3].value);
    ...
    for ( ; samples > 0; samples >>= 1) {
         ...
         * rules, this should be an accelerated one if possible */
        {
            ...
            qeglChooseConfig(qeglGetDisplay(EGL_DEFAULT_DISPLAY),
                            (const int *) attribs,
                            &config, 1,
                            &numConfigs);
...
static void m3gInitializeEGL(void)
{
    ...
    qeglInitialize(qeglGetDisplay(EGL_DEFAULT_DISPLAY), NULL, NULL);
}
...
static void m3gTerminateEGL(void)
{
    ...
    qeglTerminate(qeglGetDisplay(EGL_DEFAULT_DISPLAY));
}
...
{
    qeglDestroyContext(qeglGetDisplay(EGL_DEFAULT_DISPLAY), ctx);
#   if defined(M3G_DEBUG)
    {
        EGLint err = qeglGetError();
        if (err != EGL_SUCCESS) {
                                         ...
                                         M3GNativeWindow wnd)
{
    ...
    surf = qeglCreateWindowSurface(qeglGetDisplay(EGL_DEFAULT_DISPLAY),
                                  config,
                                         ...
                                         M3GNativeBitmap bmp)
{
    ...
    surf = qeglCreatePixmapSurface(qeglGetDisplay(EGL_DEFAULT_DISPLAY),
                                  config,
                                          ...
                                          M3Gint width, M3Gint height)
{
    ...
    surf = qeglCreatePbufferSurface(qeglGetDisplay(EGL_DEFAULT_DISPLAY),
                                   config,
                                   attrib);
#   if defined(M3G_DEBUG)
    {
...
{
    qeglDestroySurface(qeglGetDisplay(EGL_DEFAULT_DISPLAY), surface);
...
{
    EGLBoolean success = qeglSwapBuffers(qeglGetDisplay(EGL_DEFAULT_DISPLAY),
                                        surface);
...
#   define MAX_TEMP_TEXTURES    8
    ...
    GLint tempTexCount;
	//k
	GLsizei bytes_per_pixel = 4; // bytes per pixel
	...
	//k
	if(internalFormat == M3G_RGB565)
	{
        glFormat = GL_RGB;
		bytes_per_pixel = 2; // GL_UNSIGNED_SHORT_5_6_5
    ...
    if (mustConvert) {
        ...
        M3Gint maxWidth;
        qglGetIntegerv(GL_MAX_TEXTURE_SIZE, &maxWidth);
    ...
    qglEnable(GL_TEXTURE_2D);
    {
        ...
        qglGenTextures(tempTexCount, tempTexObj);
        
        for (ti = 0; ti < tempTexCount; ++ti) {
            qglBindTexture(GL_TEXTURE_2D, tempTexObj[ti]);
            qglTexEnvx(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_REPLACE);
            qglTexParameterx(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
            qglTexParameterx(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
            M3G_ASSERT_GL;
            ...
            qglTexImage2D(GL_TEXTURE_2D, 0,
                         ...
                         0,
                         glFormat,
                         data_format/*//k GL_UNSIGNED_BYTE*/, NULL);
    ...
    qglTexCoordPointer(2, GL_BYTE, 0, tc);
    qglEnableClientState(GL_TEXTURE_COORD_ARRAY);
    qglVertexPointer(2, GL_SHORT, 0, pos);
...

    {
        ...
        for (y = 0; y < height; y += tileHeight) {
            for (x = 0; x < width; x += tileWidth) {
                ...
                qglBindTexture(GL_TEXTURE_2D, tempTexObj[nextTexTile]);
                ...
                if (mustConvert) {
                    ...
                    if (w*bpp == stride) {
                        qglTexSubImage2D(GL_TEXTURE_2D, 0,
                                        ...
                                        w, h,
                                        glFormat,
                                        data_format/*//k GL_UNSIGNED_BYTE*/,
                                        pixels + y * stride + x * bpp);
                    }
                    else {
                        int k;
...
CleanUpAndExit:
    ...
    qglDeleteTextures(tempTexCount, tempTexObj);
                                     ...
                                     const M3Gubyte *pixels)
{
    ...
    qglScissor(xOffset, yOffset, width, height);
    qglViewport(0, 0, ctx->target.width, ctx->target.height);
    qglMatrixMode(GL_PROJECTION);
    qglLoadIdentity();
    qglOrthox(0, ctx->target.width << 16,
...
static void m3gUpdateTargetBuffer(RenderContext *ctx)
{
    ...
    if (ctx->target.type == SURFACE_IMAGE) {
...
#if 0
		// draw a red triangle in top-right side, for test
		{
			GLfloat ptr[] = {
				1,0,
				3,1,
			...
			qglPushMatrix();
			qglLoadIdentity();
			qglOrthof(-5, 5, -5, 5, -1, 5);
			qglMatrixMode(GL_MODELVIEW);
			qglPushMatrix();
			qglLoadIdentity();

			qglColor4f(1,0,0,1);
			qglVertexPointer(2, GL_FLOAT, 0, ptr);
        ...
        if (ctx->target.type == SURFACE_BITMAP) {
            ...
            if (fullClip && qeglCopyBuffers(qeglGetDisplay(EGL_DEFAULT_DISPLAY),
...
static EGLConfig m3gEGLConfigForConfigID(EGLDisplay display, GLint configID)
{
	...
	if (EGL_FALSE == qeglGetConfigs(display, configs, 1024, &numConfigs) )
	{
		return NULL;
	}

#if __STDC_VERSION__ >= 199901 //harmattan
...
#endif
	{
		...
		qeglGetConfigAttrib(display, configs[i], EGL_CONFIG_ID, &value);
		if (value == configID)
		{
			return configs[i];
		}
                                     ...
                                     EGLSurface surface)
{
    ...
    for (i = 0; i < M3G_MAX_GL_CONTEXTS; ++i) {
        ...
        if ((rc->surfaceTypeBits & surfaceTypeBits) == surfaceTypeBits
            ...
            if (qeglMakeCurrent(qeglGetDisplay(EGL_DEFAULT_DISPLAY), 
                               surface, surface, rc->handle)) {
                rc->lastUseTime = ctx->cacheTimeStamp;
	printf("=====eeeeeeeee %x |%s %s %s %s\n", qeglGetError(), qglGetString(GL_VERSION), qglGetString(GL_VENDOR), qglGetString(GL_EXTENSIONS), qglGetString(GL_RENDERER));
                return rc->handle;
            }
            else {
    ...
    /* No match found, we must create a new context */
    {
        ...
        //if (surfaceTypeBits == SURFACE_EGL) 
        {
            ...
            EGLint configID;
            qeglQuerySurface(dpy,
                            surface,//(EGLSurface) ctx->target.handle,
                            EGL_CONFIG_ID,
                            &configID);
            glrc = qeglCreateContext(dpy, m3gEGLConfigForConfigID(dpy, configID), shareRc, NULL);
            //M3G_ASSERT(glrc);
        }
        /*else {
        ...
        */
        ...
        {
            M3Gbool ok = qeglMakeCurrent(qeglGetDisplay(EGL_DEFAULT_DISPLAY),
...
static void m3gMakeGLCurrent(RenderContext *ctx)
{
	...
	printf(";;;;;;;;;;;;;egl;%s\n", __func__);
            EGLint param;
            qeglQueryContext(qeglGetCurrentDisplay(),
                            eglCtx, EGL_CONFIG_ID,
                            &param);
            qeglGetConfigAttrib(qeglGetCurrentDisplay(),
                               m3gEGLConfigForConfigID(qeglGetCurrentDisplay(), param), EGL_CONFIG_CAVEAT,
                               &param);
	...
	printf(";;;;;;;null;;;%s\n", __func__);
        qeglMakeCurrent(qeglGetDisplay(EGL_DEFAULT_DISPLAY), NULL, NULL, NULL);
    }
}
                                     ...
                                     M3GEGLSurface surface)
{
    ...
    M3G_LOG1(M3G_LOG_RENDERING, "Binding EGL surface 0x%08X\n", (unsigned) surface);
    {
        ...
        if (!(qeglQuerySurface(dpy, surf, EGL_WIDTH, &width) &&
              qeglQuerySurface(dpy, surf, EGL_HEIGHT, &height))) {
