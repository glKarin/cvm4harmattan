...
static void m3gApplyDefaultCompositingMode(M3Gbool alphaWrite)
{
    ...
    /* colorWrite = true */
    qglColorMask(GL_TRUE, GL_TRUE, GL_TRUE, alphaWrite);

    /* threshold = 0.0 */
    qglAlphaFunc(GL_GEQUAL, 0.0f);
...
static void m3gApplyCompositingMode(CompositingMode *compositingMode, RenderContext *ctx)
{
	...
	if (compositingMode != NULL) {
        ...
        qglDepthFunc(compositingMode->depthTest ? GL_LEQUAL : GL_ALWAYS);
        qglDepthMask(compositingMode->depthWrite);

        if (m3gGetColorMaskWorkaround(M3G_INTERFACE(compositingMode))) {
            qglColorMask(GL_TRUE, GL_TRUE, GL_TRUE, GL_TRUE);
        }
        else {
            qglColorMask(compositingMode->colorWrite,
        ...
        if (compositingMode->alphaThreshold == 0.0f) {
            ...
            qglAlphaFunc(GL_GEQUAL, compositingMode->alphaThreshold);
        ...
        qglPolygonOffset(compositingMode->factor, compositingMode->units);
