                                    ...
                                    RenderQueue *renderQueue)
{
	...
	if (caller != parent && parent != NULL) {
        ...
        success = M3G_VFUNC(Node, parent, setupRender)(parent,
                                                       self, s, renderQueue);
    printf("uuuu %d %p %d %d %d\n", caller != parent, parent, success, self->scope, parent->scope);
    }
                            ...
                            M3Gfloat clipNear, M3Gfloat clipFar)
{
    ...
    M3G_VALIDATE_OBJECT(camera);
	printf("para %d %d %d %d\n", height, aspectRatio, clipNear, clipFar);
	printf("para %f %f %f %f\n", height, aspectRatio, clipNear, clipFar);
                               ...
                               M3Gfloat clipNear, M3Gfloat clipFar)
{
	...
	//k
	printf("¡¡¡¡¡¡¡ %f %f %f %f\n", fovy, aspectRatio, clipNear, clipFar);
	if(fovy <= 0.0f)
		fovy = 0.00001f;
	if(aspectRatio <= 0.0f)
		aspectRatio = 0.00001f;
	if(clipNear <= 0.0f)
		clipNear = 0.00001f;
	if(clipFar <= 0.0f)
		clipFar = 0.00002f;
