                          ...
                          const Vec4 *spotDir)
{
    ...
    if (self->mode == M3G_DIRECTIONAL) {
        ...
        temp[2] = -spotDir->z;
        temp[3] = 0.0f;
        qglLightfv(glLight, GL_POSITION, temp);
    }
    else {
        qglLightfv(glLight, GL_POSITION, &pos->x);
        if (self->mode == M3G_SPOT) {
            qglLightfv(glLight, GL_SPOT_DIRECTION, &spotDir->x);
        }
    }
	...
	if (self->mode == M3G_AMBIENT) {
		qglLightfv(glLight, GL_AMBIENT, light);
		qglLightfv(glLight, GL_DIFFUSE, BLACK);
		qglLightfv(glLight, GL_SPECULAR, BLACK);
	}
	else {
		qglLightfv(glLight, GL_AMBIENT, BLACK);
		qglLightfv(glLight, GL_DIFFUSE, light);
		qglLightfv(glLight, GL_SPECULAR, light);
	}

    /* Set spot parameters */
    
	if (self->mode == M3G_SPOT) {
		qglLightf(glLight, GL_SPOT_EXPONENT, self->spotExponent);
		qglLightf(glLight, GL_SPOT_CUTOFF, self->spotAngle);
	}
	else {
		qglLightf(glLight, GL_SPOT_CUTOFF, 180.0f);
	}

    /* Set attenuation */
    
	if (self->mode == M3G_OMNI || self->mode == M3G_SPOT) {
		qglLightf(glLight, GL_CONSTANT_ATTENUATION,  self->constantAttenuation);
		qglLightf(glLight, GL_LINEAR_ATTENUATION,    self->linearAttenuation);
		qglLightf(glLight, GL_QUADRATIC_ATTENUATION, self->quadraticAttenuation);
	}
	else if (self->mode == M3G_AMBIENT) {
		qglLightf(glLight, GL_CONSTANT_ATTENUATION,  1.0f);
		qglLightf(glLight, GL_LINEAR_ATTENUATION,    0.0f);
		qglLightf(glLight, GL_QUADRATIC_ATTENUATION, 0.0f);
	}
}
