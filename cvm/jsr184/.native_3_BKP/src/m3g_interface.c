...
#   else /* !M3G_NGL_CONTEXT_API */
    ...
    qeglChooseConfig(qeglGetDisplay(0),
                    attrib,
                    &config, 1,
                    &numConfigs);
    ...
    ctx = qeglCreateContext(qeglGetDisplay(0),
                           config,
                           NULL,
                           NULL);
    ...
    surf = qeglCreatePbufferSurface(qeglGetDisplay(0),
                                   config,
                                   attrib);

    qeglMakeCurrent(qeglGetDisplay(0), 
                   surf, surf, ctx);
    ...
    */
	...
	karinPrintGL();
	printf("glActiveTexture symbol address: Qt/OpenGLES2 -> %p, OpenGLES1(dlopen) -> %p\n", glActiveTexture, qglActiveTexture);
	printf("glGetString symbol address: Qt/OpenGLES2 -> %p, OpenGLES1(dlopen) -> %p\n", glGetString, qglGetString);
	printf("OpenGLESv1 symbol address: glEnableClientState -> %p, glFogf -> %p, glColor4f -> %p, glVertexPointer -> %p\n", qglEnableClientState, qglFogf, qglColor4f, qglVertexPointer);
	info = qglGetString(GL_RENDERER);
...
#   endif
    ...
    qglGetIntegerv(GL_MAX_TEXTURE_SIZE, params);

    m3g->maxTextureDimension = params[0];

    qglGetIntegerv(GL_MAX_VIEWPORT_DIMS, params);

    m3g->maxViewportWidth = params[0];
    m3g->maxViewportHeight = params[1];
    m3g->maxViewportDim = M3G_MIN(params[0], params[1]);

    qeglMakeCurrent(qeglGetDisplay(0), NULL, NULL, NULL);
    qeglDestroySurface(qeglGetDisplay(0), surf);
    qeglDestroyContext(qeglGetDisplay(0), ctx);
...
#       if !defined(M3G_NGL_CONTEXT_API)
		...
		//qeglInitialize(qeglGetDisplay(EGL_DEFAULT_DISPLAY), NULL, NULL); //k init EGL: 1.4 build 1.4.14.2514 Nokia
		//karinPrintEGL(); //k

        if (qeglQueryString(qeglGetDisplay(EGL_DEFAULT_DISPLAY), EGL_VERSION)) {
            ++m3g->glRefCount;
        }
#       endif /*!M3G_NGL_CONTEXT_API*/
