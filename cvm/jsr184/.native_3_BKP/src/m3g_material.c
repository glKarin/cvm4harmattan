...
static void m3gApplyMaterial(Material *material, M3Gint alphaFactor)
{
    if (material != NULL) {
        ...
        if (material->vertexColorTracking) {
            ...
            qglMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, colors);
            ...
            qglMaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE, colors);
        }
        m3gFloatColor(material->emissiveColor, 1.0f, colors);
        qglMaterialfv(GL_FRONT_AND_BACK, GL_EMISSION, colors);
        m3gFloatColor(material->specularColor, 1.0f, colors);
        qglMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, colors);
        qglMaterialf(GL_FRONT_AND_BACK, GL_SHININESS, material->shininess);
