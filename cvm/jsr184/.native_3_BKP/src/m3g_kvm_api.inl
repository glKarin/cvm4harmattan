...
#define M3G_RAISE_EXCEPTION(aEnv, aException){\
                 if (aEnv != NULL){\
                jclass jException = (*aEnv)->FindClass(aEnv, aException);\
                if (jException != NULL){\
                    (*aEnv)->ThrowNew(aEnv, jException, NULL);\
         }}}\
...
#else
#define M3G_DO_LOCK CSynchronization__InstanceL()->Lock(CSynchronization__InstanceL());
#define M3G_DO_UNLOCK(aEnv) {\
                    int errorCode = CSynchronization__InstanceL()->GetErrorCode(CSynchronization__InstanceL());\
                    if ( errorCode != 0){\
                            M3G_RAISE_EXCEPTION(aEnv, jsr184Exception(errorCode));\
                    }\
                    CSynchronization__InstanceL()->Unlock(CSynchronization__InstanceL());\
                    }\
...
#else
    if ((*aEnv)->GetArrayLength(aEnv, aArray) < aMinLength)
#endif
    {
