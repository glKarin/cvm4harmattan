...
public class Protocol extends ProtocolNative implements FileConnection {
    ...
    public Connection prim_openProtocol(String openName, String parms, int mode) throws IOException {
        ...
        String priv = homePath + "/appdb/private";
        if (!(new File(priv).exists())) {
            new File(priv).mkdir();
        }

        String storage = homePath + "/appdb/storage";
        ...
        if (openName.startsWith("/Private")) {
            openName = homePath + "/appdb/private" + openName.substring(8);
        } else
        if (openName.startsWith("/Storage")) {
            openName = homePath + "/appdb/storage" + openName.substring(8);
        }
