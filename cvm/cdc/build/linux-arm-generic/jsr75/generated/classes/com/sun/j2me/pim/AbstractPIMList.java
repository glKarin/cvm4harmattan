...
public abstract class AbstractPIMList implements PIMList {
    ...
    public String getAttributeLabel(int attribute) {
        ...
        // ATTR_NONE==0 must be supported by Contact list
        if (type == PIM.CONTACT_LIST && attribute == Contact.ATTR_NONE) {
                return "NONE";
        }
        String label =
    ...
    public int maxCategories() {
        return PIMHandler.getInstance().getMaxCategories(handle);
    }
     ...
     * @throws   IllegalArgumentException if field not valid 
     *           (takes precedence over UnsupportedFieldException).
