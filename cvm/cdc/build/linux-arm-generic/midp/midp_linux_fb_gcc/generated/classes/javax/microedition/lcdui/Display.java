 ...
 */
    ...
    /**
    * Release any resources, such as threads, which migth refer
    * back to the application-level, which in turn may have
    * a reference to the Display.
    */
    private void close()
    {
        
        // Free the window to allow it to GC.
        window = null;
 ...
 */
    ...
    public void setCurrent(Displayable nextDisplayable) {
        ...
        // Handle null right away, to avoid risk of NullPointerException below.
        if (nextDisplayable == null) {
            setCurrentImpl(null, null);
            return;
        }
 ...
 */
    ...
    boolean ret = false; 
        ...
        // Close the display to clean-up internal resources that otherwise
        // prevent GC of application class loaders.
        das[i].close();
 ...
 */
    ...
    /** Get the width of some default alert depending on the layers attached to the screen 
     * @param withScrollBar true if the scroll bar is in use for the  displayable
     * @return width of the paticular displayable
     */
    static int getDefaultAlertWidth(boolean withScrollBar) {
    return MIDPWindow.getDefaultAlertWidth(WIDTH, withScrollBar);
    }
    ...
    /** 
     * Get the height of some default alert 
     * @return height of the paticular body layer
     */
    static int getDefaultAlertHeight() {
    return MIDPWindow.getDefaultAlertHeight(HEIGHT);
    }
    ...
    class ChameleonTunnel implements ChamDisplayTunnel {
        ...
        /**
         * This method is used by Chameleon to invoke
         * hideNotify() method from Canvas. This may call into
         * application code if the current displayable
         * is a canvas or canvas subclass.
         */
        public void callHideNotify() {
            // A copy of current LF object            
            DisplayableLF currentCopy = Display.this.current;
            if (currentCopy != null && currentCopy instanceof CanvasLF) {
                CanvasLFImpl canvasLF = (CanvasLFImpl)currentCopy;
                try {
                    synchronized (Display.calloutLock) {
                        canvasLF.canvas.hideNotify();
                    }
                } catch (Throwable thr) {
        ...
        /**
         * This method is used by Chameleon to invoke
         * showNotify methos from Canvas. This may call into
         * application code if the current displayable
         * is a canvas or canvas subclass.
         */
     ...
     */
        ...
        public void handleRotationEvent(int state) {
            // IMPL_NOTE: DisplayDevice class supports only 2 states now: landscape and portrait
            boolean isPortrait = !Display.this.displayDevice.getReverseOrientation();
            if (
                (isPortrait && 
                    (state == 
                        OrientationListener.FODDER_BANK ||
                        state == OrientationListener.BOW_ROLL)
                ) || (!isPortrait &&
                    (state == 
                        OrientationListener.LEFT_BANK ||
                        state == OrientationListener.RIGHT_BANK)
                )) {
                return;
            }
