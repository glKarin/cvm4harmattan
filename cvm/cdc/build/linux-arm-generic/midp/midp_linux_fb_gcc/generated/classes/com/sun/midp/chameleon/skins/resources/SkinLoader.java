...
public class SkinLoader {
   ...
   /**
     ...
     * @param reload if true, ignore previously loaded resources and reload
     */
    private static void loadResources(boolean loadAll, boolean reload) {
        // load selected resources
        ScreenResources.load(reload);
        ScrollIndResources.load(reload);
        SoftButtonResources.load(reload);
        TickerResources.load(reload);
        TitleResources.load(reload);
        AlertResources.load(reload);


        // load the rest of resources
        if (loadAll) {
            PTIResources.load(reload);
            InputModeResources.load(reload);
            BusyCursorResources.load(reload);
            ChoiceGroupResources.load(reload);
            DateEditorResources.load(reload);
            DateFieldResources.load(reload);
            GaugeResources.load(reload);
            ImageItemResources.load(reload);
            MenuResources.load(reload);
            ProgressBarResources.load(reload);
            StringItemResources.load(reload);
            TextFieldResources.load(reload);
            UpdateBarResources.load(reload);
            VirtualKeyboardResources.load(reload);
        }
